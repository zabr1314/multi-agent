...
| 策略 | 方式 | 特点 | 何时用 |
|:---|:---|:---|:---|
| spawn | 指定 subagent_type | 独立上下文，不共享 cache | 需要完全独立的判断 |
| fork | 省略 subagent_type | 共享父级 cache，便宜 | 研究类任务，噪音大的工作 |
...
Claude Code 内置 6 种 agent，每种有不同角色：

| Agent | 能力 | 模型 | 特点 |
|:---|:---|:---|:---|
| General Purpose | 所有工具 | 默认 | 通用执行 |
| Explore | 只读搜索 | haiku | 快速，禁写入 |
| Plan | 只读规划 | inherit | 输出 Critical Files |
| Verification | 对抗验证 | inherit | PASS/FAIL/PARTIAL |
| Guide | 文档查询 | haiku | 查官方文档 |
| Statusline | Read+Edit | sonnet | 配置状态栏 |

Agent 生命周期：创建 → 初始化 MCP/hooks/skills → 执行 → 清理（finally 块清理 8 种资源）
...
Verification Agent 的对抗性编排：预判模型的自我欺骗（验证逃避、80% 魅惑），用结构化输出强制证据。
...
- **Agent 间通信用 SendMessage** — 文本输出对其他 agent 不可见
- **Verification 必须独立** — 自我验证不可靠，必须另起 agent
